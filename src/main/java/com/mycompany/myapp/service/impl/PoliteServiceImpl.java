...
public class PoliteServiceImpl implements PoliteService {
    ...
    public Page<PoliteDTO> findAll(Pageable pageable) {
        log.debug("Request to get all Polites");
        return politeRepository.findAll(pageable).map(politeMapper::toDto);
    }

    /**
     *  Get all the polites where Tranzactii is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<PoliteDTO> findAllWhereTranzactiiIsNull() {
        log.debug("Request to get all polites where Tranzactii is null");
        return StreamSupport
            .stream(politeRepository.findAll().spliterator(), false)
            .filter(polite -> polite.getTranzactii() == null)
            .map(politeMapper::toDto)
