...
public class PoliteServiceImpl implements PoliteService {
    ...
    /**
     *  Get all the polites where T is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<PoliteDTO> findAllWhereTIsNull() {
        log.debug("Request to get all polites where T is null");
        return StreamSupport
            .stream(politeRepository.findAll().spliterator(), false)
            .filter(polite -> polite.getT() == null)
            .map(politeMapper::toDto)
