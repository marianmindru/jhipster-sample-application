...
public class OferteServiceImpl implements OferteService {
    ...
    public Page<OferteDTO> findAll(Pageable pageable) {
        log.debug("Request to get all Ofertes");
        return oferteRepository.findAll(pageable).map(oferteMapper::toDto);
    }

    /**
     *  Get all the ofertes where Tranzactii is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<OferteDTO> findAllWhereTranzactiiIsNull() {
        log.debug("Request to get all ofertes where Tranzactii is null");
        return StreamSupport
            .stream(oferteRepository.findAll().spliterator(), false)
            .filter(oferte -> oferte.getTranzactii() == null)
            .map(oferteMapper::toDto)
