...
public class OferteServiceImpl implements OferteService {
    ...
    /**
     *  Get all the ofertes where Tranzactie is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<OferteDTO> findAllWhereTranzactieIsNull() {
        log.debug("Request to get all ofertes where Tranzactie is null");
        return StreamSupport
            .stream(oferteRepository.findAll().spliterator(), false)
            .filter(oferte -> oferte.getTranzactie() == null)
            .map(oferteMapper::toDto)
