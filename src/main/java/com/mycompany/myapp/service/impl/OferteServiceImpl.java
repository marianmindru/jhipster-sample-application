...
public class OferteServiceImpl implements OferteService {
    ...
    /**
     *  Get all the ofertes where T is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<OferteDTO> findAllWhereTIsNull() {
        log.debug("Request to get all ofertes where T is null");
        return StreamSupport
            .stream(oferteRepository.findAll().spliterator(), false)
            .filter(oferte -> oferte.getT() == null)
            .map(oferteMapper::toDto)
