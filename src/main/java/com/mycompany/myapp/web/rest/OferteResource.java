...
public class OferteResource {
     ...
     */
    @GetMapping("/ofertes")
    public ResponseEntity<List<OferteDTO>> getAllOfertes(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("tranzactii-is-null".equals(filter)) {
            log.debug("REST request to get all Ofertes where tranzactii is null");
            return new ResponseEntity<>(oferteService.findAllWhereTranzactiiIsNull(), HttpStatus.OK);
        }
        log.debug("REST request to get a page of Ofertes");
        Page<OferteDTO> page = oferteService.findAll(pageable);
