...
public class PoliteResource {
     ...
     */
    @PostMapping("/polites")
    public ResponseEntity<PoliteDTO> createPolite(@RequestBody PoliteDTO politeDTO) throws URISyntaxException {
    ...
    public ResponseEntity<PoliteDTO> updatePolite(
        ...
        @RequestBody PoliteDTO politeDTO
     ...
     */
    @GetMapping("/polites")
    public ResponseEntity<List<PoliteDTO>> getAllPolites(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("tranzactii-is-null".equals(filter)) {
            log.debug("REST request to get all Polites where tranzactii is null");
            return new ResponseEntity<>(politeService.findAllWhereTranzactiiIsNull(), HttpStatus.OK);
        }
        log.debug("REST request to get a page of Polites");
        Page<PoliteDTO> page = politeService.findAll(pageable);
