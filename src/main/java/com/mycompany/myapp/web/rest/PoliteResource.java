...
public class PoliteResource {
     ...
     */
    @GetMapping("/polites")
    public List<PoliteDTO> getAllPolites(@RequestParam(required = false) String filter) {
        if ("tranzactie-is-null".equals(filter)) {
            log.debug("REST request to get all Polites where tranzactie is null");
            return politeService.findAllWhereTranzactieIsNull();
        }
