...
class PoliteResourceIT {
    ...
    void checkNumarPolitaIsRequired() throws Exception {
        int databaseSizeBeforeTest = politeRepository.findAll().size();
        // set the field null
        polite.setNumarPolita(null);

        // Create the Polite, which fails.
        PoliteDTO politeDTO = politeMapper.toDto(polite);
        ...
        List<Polite> politeList = politeRepository.findAll();
        assertThat(politeList).hasSize(databaseSizeBeforeTest);
    }
